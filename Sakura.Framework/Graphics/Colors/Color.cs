...
/// Inherited from <see cref="System.Drawing.Color"/> with some modifications.
/// </summary>
public readonly struct Color : IEquatable<Color>
{
    ...
    private static void CheckByte(int value, string name)
    {
        static void ThrowOutOfByteRange(int v, string n) =>
            throw new ArgumentException($"Value of '{n}' ({v}) is out of range. Must be between 0 and 255 inclusive.", n);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static implicit operator SystemDrawingColor(Color color)
    {
        if (color.IsEmpty)
            return SystemDrawingColor.Empty;

        if (color.IsKnownColor)
            return SystemDrawingColor.FromKnownColor((System.Drawing.KnownColor)color.ToKnownColor());

        return SystemDrawingColor.FromArgb(color.A, color.R, color.G, color.B);
    }
}
